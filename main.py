...
# 1. APIキーの設定（セキュリティ対策：クラウドのSecretsから読み込みます）
try:
    API_KEY = st.secrets["GEMINI_API_KEY"]
except:
    # ローカルPCテスト用（クラウド公開後はSecretsが優先されます）
...
# 契約時の持ち物選択（状況5）
    ...
    items_list = ["身分証明書", "履歴事項全部証明書", "収入証明書", "住民票", "印監証明書", "認印", "実印", "初期費用（持参・振込）", "仲介手数料（持参・振込）"]
        ...
        with st.spinner("作成中..."):
            try:
                ...
                if status == "3. 内見日時の打診（日程の提案・相談）":
                    ...
                    scenario_instruction = f"契約案内です。店舗（堺市北区新金岡町）へお越しいただく旨を伝え、以下の持ち物を明記してください。{extra_info}"
...
2. 件名：物件名のみの形式（例：{prop_name_input} のお問い合わせについて）。人名・社名・担当者名は含めない。
3. 本文：物件名、賃料、面積を必ず明記。補足内容が空欄でも、選択状況にふさわしい丁寧な文面をゼロから作成すること。
4. 結び：必ず「引き続きよろしくお願い申し上げます。」で締める。
5. 署名：
なごみ不動産
担当：{staff_name}
